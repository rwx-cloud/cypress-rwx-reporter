...
class RwxReporter {
  ...
  recordAttempt(test, statusKind, error) {
    ...
    if (this.reporterOptions.adjustAttemptFile) {
      const fullAdjustAttemptFile = this.projectRoot + '/' + this.reporterOptions.adjustAttemptFile;
      const adjustAttempt = require(fullAdjustAttemptFile).default || require(fullAdjustAttemptFile);
